...
SEXP AlignedXStringSet_align_aligned(SEXP alignedXStringSet, SEXP gapCode)
{
	...
	XStringSet_holder unaligned_holder = hold_XStringSet(unaligned);
	...
	int numberOfStrings = get_XStringSet_length(unaligned);
	...
	for (i = 0, rangeStart = INTEGER(get_IRanges_start(range)), rangeWidth = INTEGER(get_IRanges_width(range));
		...
		Chars_holder origString = get_elt_from_XStringSet_holder(&unaligned_holder, stringElement);
...
SEXP PairwiseAlignmentsSingleSubject_align_aligned(SEXP alignment, SEXP gapCode, SEXP endgapCode)
{
	...
	XStringSet_holder unalignedPattern_holder = hold_XStringSet(unalignedPattern);
	...
	for (i = 0,
		...
		Chars_holder origString = get_elt_from_XStringSet_holder(&unalignedPattern_holder, i);
