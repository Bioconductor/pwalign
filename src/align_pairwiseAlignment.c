		...
		SEXP fuzzyLookupTable)
{
	...
	XStringSet_holder pattern_holder = hold_XStringSet(pattern);
	XStringSet_holder subject_holder = hold_XStringSet(subject);
	const int numberOfStrings = get_length_from_XStringSet_holder(&pattern_holder);
	const int multipleSubjects = get_length_from_XStringSet_holder(&subject_holder) > 1;
	...
	if (useQualityValue) {
		...
		patternQuality_holder = hold_XStringSet(patternQuality);
		lengthOfPatternQualitySet = get_XStringSet_length(patternQuality);
		subjectQuality_holder = hold_XStringSet(subjectQuality);
		lengthOfSubjectQualitySet = get_length_from_XStringSet_holder(&subjectQuality_holder);
	} else {
	...
	align2Info.string = get_elt_from_XStringSet_holder(&subject_holder, 0);
	if (useQualityValue)
		align2Info.quality = get_elt_from_XStringSet_holder(&subjectQuality_holder, 0);
	align1Info.endGap =
	...
	if (multipleSubjects) {
		for (i = 0; i < numberOfStrings; i++) {
			int nchar1 = get_elt_from_XStringSet_holder(&pattern_holder, i).length;
			int nchar2 = get_elt_from_XStringSet_holder(&subject_holder, i).length;
		...
		for (i = 0; i < numberOfStrings; i++) {
			nCharString1 = MAX(nCharString1, get_elt_from_XStringSet_holder(&pattern_holder, i).length);
		}
	...
	if (scoreOnlyValue) {
		...
		for (i = 0, score = REAL(output); i < numberOfStrings; i++, score++) {
			...
			align1Info.string = get_elt_from_XStringSet_holder(&pattern_holder, i);
			if (useQualityValue) {
				align1Info.quality = get_elt_from_XStringSet_holder(&patternQuality_holder, quality1Element);
				quality1Element += quality1Increment;
			}
			if (multipleSubjects) {
				align2Info.string = get_elt_from_XStringSet_holder(&subject_holder, i);
				if (useQualityValue) {
					align2Info.quality = get_elt_from_XStringSet_holder(&subjectQuality_holder, quality2Element);
		...
		SEXP fuzzyLookupTable)
{
	...
	int numberOfStrings = get_XStringSet_length(string);
	int lengthOfStringQualitySet = 0;

	SEXP stringQuality = R_NilValue;
	XStringSet_holder string_holder = hold_XStringSet(string);
	XStringSet_holder stringQuality_holder;
	if (useQualityValue) {
		stringQuality = GET_SLOT(string, install("quality"));
		stringQuality_holder = hold_XStringSet(stringQuality);
		lengthOfStringQualitySet = get_XStringSet_length(stringQuality);
	}
	...
	int nCharString = 0;
	for (i = 0; i < numberOfStrings; i++) {
		nCharString = MAX(nCharString, get_elt_from_XStringSet_holder(&string_holder, i).length);
	}
	...
	if (!useQualityValue) {
		for (i = 0; i < numberOfStrings; i++) {
	        R_CheckUserInterrupt();
			align1Info.string = get_elt_from_XStringSet_holder(&string_holder, i);
			for (j = i + 1; j < numberOfStrings; j++) {
				align2Info.string = get_elt_from_XStringSet_holder(&string_holder, j);
				*score = pairwiseAlignment(
						&align1Info,
						&align2Info,
						localAlignment,
		...
		for (i = 0; i < numberOfStrings; i++) {
	        R_CheckUserInterrupt();
			align1Info.string = get_elt_from_XStringSet_holder(&string_holder, i);
			align1Info.quality = get_elt_from_XStringSet_holder(&stringQuality_holder, iQualityElement);
			jQualityElement = iQualityElement + qualityIncrement;
			iQualityElement += qualityIncrement;
			for (j = i + 1; j < numberOfStrings; j++) {
				align2Info.string = get_elt_from_XStringSet_holder(&string_holder, j);
				align2Info.quality = get_elt_from_XStringSet_holder(&stringQuality_holder, jQualityElement);
